...
const Dashboard = () => {
		...
		try {
			...
			if (user?.role === "worker") {
				try {
					...
					console.error(
						"Failed to fetch worker dashboard via service:",
						err
					);
				}
			} else if (endpoint) {
	...
	const renderActiveSection = () => {
		...
		if (
			...
			console.log(
				"🔐 Contract sign route detected, contractId:",
				contractId
			);
				...
				return (
					<div className="text-center py-8">
						...
						<p className="text-muted-foreground">
							No job ID provided for progress update.
						</p>
		...
		if (activeSection === "job-details") {
			...
			console.log(
				"🎯 Loading JobDetails with jobId from URL:",
				jobIdFromUrl
			);
		...
		switch (activeSection) {
			...
			default:
				if (
					...
					return (
						<JobDetails jobId={jobIdFromUrl} key={jobIdFromUrl} />
					);
				}
		...
		if (user.role === "admin" || user.role === "verifier") {
			return [
				...
				},
				{
					...
					label: "Revenue",
					value: `₦${(
						dashboardStats.platformRevenue || 0
					).toLocaleString()}`,
					icon: DollarSign,
					color: "text-purple-600",
					bgColor: "bg-purple-100",
	...
	return (
        ...
        `}
				...
				{/* Main Content Area */}
				<div className="w-full min-h-screen p-6 lg:p-8 pt-20">
					...
					<div className="bg-white rounded-2xl border mt-5 p-3 border-slate-200/60 shadow-sm overflow-hidden">
						{renderActiveSection()}
					</div>
				</div>
			</main>
