...
export const WalletManagement = () => {
	...
	const checkWalletStatus = async () => {
		...
		try {
			const walletResponse = await fetch(
				"https://verinest.up.railway.app/api/wallet",
				{
					headers: {
	...
	const fetchBankAccounts = async () => {
		try {
			const response = await fetch(
				"https://verinest.up.railway.app/api/wallet/bank-accounts",
				{
					headers: {
						Authorization: `Bearer ${token}`,
						"Content-Type": "application/json",
	...
	const fetchTransactions = async () => {
		try {
			const response = await fetch(
				"https://verinest.up.railway.app/api/wallet/transactions",
				{
					headers: {
						Authorization: `Bearer ${token}`,
						"Content-Type": "application/json",
			...
			if (response.ok) {
				...
				if (data.data && Array.isArray(data.data)) {
					...
					console.warn(
						"⚠️ Unexpected transactions response structure:",
						data
					);
					transactionsData = [];
				}
				...
				const transformedTransactions = Array.from(
	...
	const mapTransactionType = (
		...
		if (typeString.includes("deposit") || typeString.includes("credit"))
			return "deposit";
		if (typeString.includes("withdraw") || typeString.includes("debit"))
			return "withdrawal";
	...
	const mapTransactionStatus = (
		...
		if (
			statusString.includes("success") ||
			statusString.includes("completed") ||
			statusString.includes("approved")
		)
			return "success";
		if (
			statusString.includes("fail") ||
			statusString.includes("error") ||
			statusString.includes("rejected")
		)
			return "failed";
	...
	const createWallet = async () => {
		...
		try {
			const response = await fetch(
				"https://verinest.up.railway.app/api/wallet/create",
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (response.ok) {
				...
				if (
					...
					throw new Error(
						responseData.message || "Failed to create wallet"
					);
				}
			}
		} catch (error: any) {
	...
	const handleDeposit = async (e: React.FormEvent) => {
		...
		try {
			...
			const response = await fetch(
				"https://verinest.up.railway.app/api/wallet/deposit",
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (response.ok) {
				...
				if (paymentData.payment_url) {
					...
					if (paystackWindow) {
						toast.success(
							"Payment window opened. You can close this tab - we'll notify you when payment is confirmed."
						);
	...
	const startFallbackPolling = (reference: string) => {
		const pollInterval = setInterval(async () => {
			try {
				...
				const stillPending = localStorage.getItem(
					"pending_payment_reference"
				);
	...
	const handleWithdrawRequest = async (requestBody: any) => {
		try {
			const response = await fetch(
				"https://verinest.up.railway.app/api/wallet/withdraw",
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
			...
			if (
				...
				responseData.message.includes("transaction_pin")
			) {
			...
			if (!response.ok) {
				...
				if (
					response.status === 404 &&
					responseText.includes("bank account not found")
				) {
					throw new Error(
						"Bank account not found. Please check the account details."
					);
					...
					throw new Error(
						"Insufficient permissions. Please verify your account."
					);
	...
	const handleTransferRequest = async (requestBody: any) => {
		const response = await fetch(
			"https://verinest.up.railway.app/api/wallet/transfer",
			{
				method: "POST",
				headers: {
					Authorization: `Bearer ${token}`,
		...
		if (response.ok) {
			...
			if (
				response.status === 404 &&
				responseData.message === "Recipient not found"
			) {
				throw new Error(
				...
				throw new Error(
					responseData.message || "Invalid transfer request"
				);
			} else if (response.status === 422) {
				throw new Error(
					"Validation failed. Please check the recipient details."
				);
			} else {
				throw new Error(
					responseData.message ||
		...
		try {
			...
			if (response.ok) {
				...
				if (response.status === 422 && responseData.errors) {
					...
					throw new Error(
						responseData.message ||
							`Failed to add bank account (${response.status})`
					);
				}
			}
		} catch (error: any) {
	...
	const handleSetPrimaryAccount = async (accountId: string) => {
		try {
			const response = await fetch(
				`https://verinest.up.railway.app/api/wallet/bank-accounts/${accountId}/primary`,
				{
					method: "PUT",
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const getStatusBadge = (status: any) => {
		...
		return (
			<Badge
				variant={
					variants[statusString as keyof typeof variants] ||
					"secondary"
				}
			>
				{statusString}
	...
	const getTypeIcon = (type: any) => {
		...
		const typeString =
			typeof type === "string" ? type.toLowerCase() : "other";
			...
			<div className="space-y-6">
				<Card>
					...
					<CardContent className="space-y-4">
						<Alert>
							<AlertDescription>
								You need to create a wallet to deposit,
								withdraw, and transfer funds. Your wallet will
								be securely created and ready to use.
							</AlertDescription>
						</Alert>
						<Button
				...
				<Card className="bg-muted/50">
					...
					<CardContent>
						<p className="text-sm text-muted-foreground">
							Wallet created:{" "}
							{walletData?.wallet_created ? "Yes" : "No"}
							<br />
							Wallet ID: {walletData?.id || "Not set"}
							<br />
				...
				<CardContent>
					...
					{walletData.account_number && (
						<div className="mt-4 p-3 bg-muted rounded-lg">
							...
							<p className="text-sm text-muted-foreground">
								Account: {walletData.account_number} •{" "}
				...
				<TabsContent value="deposit">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Add money to your wallet using any payment
								method
							</CardDescription>
						</CardHeader>
						<CardContent>
							...
							>
								...
								<div className="space-y-2">
									...
									<Input
										...
										value={
											transferData.recipient_identifier
										}
										onChange={(e) =>
											setTransferData({
												...transferData,
				...
				<TabsContent value="trust_point">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Your financial trustworthiness and reliability
								rating
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-6">
								...
								<div className="text-center space-y-4">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Your trust score is in the top 20%
											of users
										</p>
									</div>
								</div>
								...
								<div className="space-y-4">
									...
									<div className="space-y-2">
										...
										<div className="flex items-center gap-2 text-sm">
											<div className="w-2 h-2 bg-primary rounded-full"></div>
											<span>
												Maintain consistent transaction
												activity
											</span>
										</div>
										<div className="flex items-center gap-2 text-sm">
