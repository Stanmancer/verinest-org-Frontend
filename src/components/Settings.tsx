...
const PUBLIC_BASE_URL =
	import.meta.env.VITE_PUBLIC_URL || "https://verinest.com";
...
export const Settings = () => {
	...
	const handleUpgrade = async () => {
		...
		try {
			...
			toast.success(
				"Payment initiated. Please confirm with your transaction PIN."
			);
	...
	const handleRoleUpgrade = async (newRole: AllowedRole) => {
		...
		try {
			...
			if (newRole === "vendor") {
				toast.error(
					"Please contact support or use the Vendor onboarding flow to become a Vendor."
				);
				setRoleLoading(false);
				return;
			}
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									...
									others see you on the platform
								</CardDescription>
							</CardHeader>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									{referralLink && (
										<div className="space-y-2">
											...
											<p className="text-xs text-muted-foreground">
												Send this link to onboard new
												users and track referrals.
											</p>
										</div>
									)}
					...
					<TabsContent value="profile" className="space-y-6">
						{publicProfileUrl && (
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Share your worker profile with employers
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									View the fields that the backend sends so
									you can confirm completeness.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									{[
										...
										{
											label: "NIN",
											value: user?.nin_number || "N/A",
										},
										{
											label: "Document Verified",
											value: user?.document_verified
												? "Yes"
												: "No",
										},
										{
					...
					<TabsContent value="role" className="space-y-6">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Choose your role: Worker, Employer, or
									Vendor.
								</CardDescription>
							</CardHeader>
							<CardContent>
											...
											>
												...
												<CardContent>
													...
													>
														{user?.role === role
															? "Current Role"
															: `Become ${role.charAt(0).toUpperCase() + role.slice(1)}`}
													</Button>
												</CardContent>
											</Card>
										))}
					...
					<TabsContent value="transaction-pin" className="space-y-6">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Manage your 6-digit PIN for authorizing
								...
								{user?.transaction_pin_set ? (
									<div className="space-y-4">
										<Alert>
											<Shield className="h-4 w-4" />
											<AlertDescription>
												Your transaction PIN is set up
												and active.
											</AlertDescription>
										</Alert>
									...
									<div className="space-y-4">
										<Alert>
											<Shield className="h-4 w-4" />
											<AlertDescription>
												You haven't set up a transaction
												PIN yet. Set one up to secure
